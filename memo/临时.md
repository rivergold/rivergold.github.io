...
## C++ OpenCV error `Unable to stop the stream: Inappropriate ioctl for device`

Rebuild OpenCV with `FFMPEG`

***References:***

- [stackoverflow: opencv Unable to stop the stream: Inappropriate ioctl for device](https://stackoverflow.com/a/45893821/4636081)

## C++ calculate run time

***References:***

- [stackoverflow: Easily measure elapsed time](https://stackoverflow.com/a/2808527/4636081)
...
## OpenCV use mouse draw rectange

***References:***

- [CSDN: OPENCV图像处理基础（五）鼠标事件画个框](https://blog.csdn.net/chenpidaxia/article/details/50975924)
...
Caffe Interp result is different from PyTorch and OpenCV result. PyTorch `upsampling` result is same with OpenCV `resize("bilinear")`.
...
```
Convolution <layer name> <unknow> <unknow> <input blob name> 0=<c_cout> 1=<kernel size> 2=<unknow> 3=<stride> 4=<padding> 5=<unknow>
```

### Problems & Solutions

#### When use `ncnn::Net.load`, occur error `find_blob_index_by_name data_splitncnn_14 failed` and `segmentation fault (core dumped)`

You need to change `input layer` of Caffe

***References:***

- [Github Tencent/ncnn MobileNetSSD_deploy.caffemodel转成bin,param之后，iOS运行时load_param出错。 #168](https://github.com/Tencent/ncnn/issues/168)

#### When build code with `#include "net.h"`, occur error `undefined reference to symbol 'GOMP_parallel@@GOMP_4.0'`

Add `-fopenmp` to `LDFLAGS` in your `CMakeLists.txt`.
...
- [stackoverflow: Makefile error causes undefined reference to symbol 'GOMP_parallel@@GOMP_4.0'](https://stackoverflow.com/questions/40428108/makefile-error-causes-undefined-reference-to-symbol-gomp-parallelgomp-4-0)
- [stackoverflow: CMake: How to set the LDFLAGS in CMakeLists.txt?](https://stackoverflow.com/questions/6077414/cmake-how-to-set-the-ldflags-in-cmakelists-txt)